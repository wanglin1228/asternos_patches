/* An hwmon driver for Asterfusion x104s-xgt-M CPLD I2C Module */
...
static ssize_t cpld_byte_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(x104s_xgt_i2c_client, attr->index);
    }
...
static ssize_t sfp_tx_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(x104s_xgt_i2c_client, SFP_TX_STAT);
...
static ssize_t sfp_tx_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    status = i2c_smbus_write_byte_data(x104s_xgt_i2c_client, SFP_TX_STAT, status);
    return count;
}
...
static ssize_t rst_ctl_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(x104s_xgt_i2c_client, RST_CTL);
    GET_BIT(status, attr->index, value);
...
static ssize_t rst_ctl_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    status = i2c_smbus_write_byte_data(x104s_xgt_i2c_client, RST_CTL, status);
    return count;
}
static ssize_t cpld_ctrl1_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(x104s_xgt_i2c_client, CPLD_CTL1);
    GET_BIT(status, attr->index, value);
...
static ssize_t cpld_ctrl1_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    status = i2c_smbus_write_byte_data(x104s_xgt_i2c_client, CPLD_CTL1, status);
    return count;
}
...
static ssize_t fan_stat_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(x104s_xgt_i2c_client, FAN_STAT);
    GET_BIT(status, attr->index, value);
...
static ssize_t fan_mode_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (FAN_CTL1 == attr->index)
    {
        status = i2c_smbus_write_byte_data(x104s_xgt_i2c_client, attr->index, val & 0x1);
    }
...
static ssize_t fan_speed_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (FAN_CTL2 == attr->index)
    {
        status = i2c_smbus_write_byte_data(x104s_xgt_i2c_client, attr->index, val % 8);
    }
...
static ssize_t sys_led_ctrl_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(x104s_xgt_i2c_client, SYS_LED); //to get register 0x30 0x13
        status &= 0x1;
...
static ssize_t loc_led_ctrl_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(x104s_xgt_i2c_client, LOC_LED); //to get register 0x30 0x13
        status &= 0x1;
...
static ssize_t hw_reset_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(x104s_xgt_i2c_client, HW_RESET); //to get register 0x30 0x13
        status &= 0x1;
...
static int Asterfusion_i2c_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
    ...
    status = sysfs_create_group(&client->dev.kobj, &X104S_XGT_SYS_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X104S_XGT_Reset_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X104S_XGT_Sensor_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X104S_XGT_Led_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X104S_XGT_SFP_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X104S_XGT_FAN_group);
    if (status)
    {
        goto exit_free;
    }
    ...
    return 0;
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &X104S_XGT_SYS_group);
    sysfs_remove_group(&client->dev.kobj, &X104S_XGT_Reset_group);
    sysfs_remove_group(&client->dev.kobj, &X104S_XGT_Sensor_group);
    sysfs_remove_group(&client->dev.kobj, &X104S_XGT_Led_group);
    sysfs_remove_group(&client->dev.kobj, &X104S_XGT_SFP_group);
    sysfs_remove_group(&client->dev.kobj, &X104S_XGT_FAN_group);
...
static struct i2c_board_info X104S_XGT_i2c_info[] __initdata =
{
    {
        I2C_BOARD_INFO("x104s_xgt", 0x40),
        .platform_data = NULL,
    },
};
...
static int __init Asterfusion_i2c_init(void)
{
    ...
    x104s_xgt_i2c_client = i2c_new_client_device(i2c_adap, &X104S_XGT_i2c_info[0]);
    if (x104s_xgt_i2c_client == NULL)
    {
        printk("ERROR: x104s_xgt_i2c_client FAILED!\n");
        return -1;
    }
...
{
    i2c_unregister_device(x104s_xgt_i2c_client);
    i2c_del_driver(&Asterfusion_i2c_driver);
}

MODULE_AUTHOR("Asterfusion Inc.");
MODULE_DESCRIPTION("Asterfusion X104S-XGT-M i2c Driver");
MODULE_LICENSE("GPL");
