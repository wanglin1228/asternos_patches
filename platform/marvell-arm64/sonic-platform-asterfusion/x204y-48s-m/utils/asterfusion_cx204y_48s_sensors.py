#!/usr/bin/python3
from __future__ import print_function
try:

    import sys
    import os
    import logging
    import json
    import subprocess
    from subprocess import call
    import click
    import math
    import commands

except ImportError as e:
    raise ImportError("%s - required module not found" % str(e))

MAX_FAN_NUM = 3
MAX_TEMP_NUM = 2

FAN_LIST = ['fan1', 'fan2', 'fan3']
TEMP_LIST = ['CPU_LM75', 'SWITCH_LM75']
TEMP_DICT = {
    'CPU_LM75': 'hwmon2/temp1_input',
    'SWITCH_LM75': 'hwmon3/temp1_input'
}

TEMP_PATH        = '/sys/class/hwmon/'
FAN_PATH         = '/sys/bus/i2c/devices/0-0030/'



# Get sysfs attribute
def get_attr_value(attr_path):
    retval = 'ERR'
    if not os.path.isfile(attr_path):
        return retval

    try:
        with open(attr_path, 'r') as fd:
            retval = fd.read()
    except Exception as error:
        logging.error("Unable to open ", attr_path, " file !")

    retval = retval.rstrip('\r\n')
    fd.close()
    return retval

def sensors_temp():
    print('TEMPERATURE SENSORS:')
    for x in TEMP_LIST:
        path = TEMP_DICT.get(x)
        result = get_attr_value(TEMP_PATH + path)
        temp = '%.2f' % (int(result, 10)/1000.0)
        print("    {} is {} degrees(C)".format(x, temp))
    print('')
    return


def fan_speed():
    print('FAN SPEED:')
    for x in FAN_LIST:
        result = get_attr_value(FAN_PATH + x)
        speed = int(result, 16) * 120
        print("    {} is {} RPM".format(x, speed))
    print('')
    return

# ==================== CLI commands and groups ====================

# This is our main entrypoint - the main 'environment' command
@click.group()
def cli():
    """environment - Command line utility for power voltage fans temps read set"""
    pass

# 'show' subgroup
@cli.group()
def show():
    """Display status of platform environment"""
    pass

# 'environment' subcommand

@show.command()
def fans():
    """Display Platform environment fans"""
    fan_speed()

@show.command()
def temps():
    """Display Platform environment temps"""
    sensors_temp()

if __name__ == "__main__":
    cli()
