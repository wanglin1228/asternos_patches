...
LED_PATH = '/sys/bus/i2c/devices/0-0030/'
FAN_PATH = '/sys/bus/i2c/devices/0-0030/'
SENSORS_PATH = '/sys/class/hwmon/hwmon2/'
...
def readFile(path):
	if FILE_LOCK.acquire():
		try:
			...
			print("Error: readFile unable to open file: %s" % str(e))
			FILE_LOCK.release()
			return 'Error'
...
def writeFile(path, value):
	if FILE_LOCK.acquire():
		try:
			...
			print("Error: writeFile unable to open file: %s" % str(e))
			FILE_LOCK.release()
			return 'Error'
