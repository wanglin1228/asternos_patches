...
static struct mutex aster_hwmon_mcu_lock;

static int aster_hwmon_mcu_read(struct i2c_client *client, u8 reg)
{
    int data = -EPERM;
    
    mutex_lock(&aster_hwmon_mcu_lock);

    data = i2c_smbus_read_word_data(client, reg);
    
    mutex_unlock(&aster_hwmon_mcu_lock);

    return data;
}

static int aster_hwmon_mcu_write(struct i2c_client *client, u8 reg, u8 val)
{
    int ret = -EIO;
             ...
             const char *buf, size_t count)
{
    ...
    if(val)
        aster_hwmon_mcu_write(client, reg, FW_UPGRADE_COMMAND);
    else
        aster_hwmon_mcu_write(client, reg, 0xff);
             ...
             char *buf)
{
    ...
    data = aster_hwmon_mcu_read(client, reg);
             ...
             const char *buf, size_t count)
{
    ...
    aster_hwmon_mcu_write(client, reg, val);
             ...
             const char *buf, size_t count)
{
    ...
    aster_hwmon_mcu_write(client, reg, data);
             ...
             const char *buf, size_t count)
{
    ...
    if(val)
        aster_hwmon_mcu_write(client, reg, FAN_ENABLE_COMMAND);
    else
        aster_hwmon_mcu_write(client, reg, FAN_DISABLE_COMMAND);
             ...
             const char *buf, size_t count)
{
    ...
    if(val)
        aster_hwmon_mcu_write(client, reg, FAN_LED_SETTO_AUTO_COMMAND);
    else
        aster_hwmon_mcu_write(client, reg, FAN_LED_SETTO_MANUAL_COMMAND);
             ...
             const char *buf, size_t count)
{
    ...
    if(val)
        aster_hwmon_mcu_write(client, reg, FAN_LED_GREENON_COMMAND);
    else
        aster_hwmon_mcu_write(client, reg, FAN_LED_GREENOFF_COMMAND);
             ...
             const char *buf, size_t count)
{
    ...
    if(val)
        aster_hwmon_mcu_write(client, reg, FAN_LED_AMBERON_COMMAND);
    else
        aster_hwmon_mcu_write(client, reg, FAN_LED_AMBEROFF_COMMAND);
...
SET_FAN_ATTR(2);SET_FAN_ATTR(3);SET_FAN_ATTR(4);SET_FAN_ATTR(5);
...
SET_ADC_ATTR(1);SET_ADC_ATTR(2);SET_ADC_ATTR(3);SET_ADC_ATTR(4);SET_ADC_ATTR(5);SET_ADC_ATTR(6);SET_ADC_ATTR(7);SET_ADC_ATTR(8);SET_ADC_ATTR(9);
...
static SENSOR_DEVICE_ATTR(lm75_48_temp_alert,  S_IRUGO, get_temp_alert, NULL, 7);
static SENSOR_DEVICE_ATTR(lm75_49_temp_alert,  S_IRUGO, get_temp_alert, NULL, 6);
static SENSOR_DEVICE_ATTR(lm75_4a_temp_alert,  S_IRUGO, get_temp_alert, NULL, 5);

static SENSOR_DEVICE_ATTR(i2c_lm75_48_timeout,  S_IRUGO, get_i2c_timeout, NULL, 5);
static SENSOR_DEVICE_ATTR(i2c_lm75_49_timeout,  S_IRUGO, get_i2c_timeout, NULL, 4);
static SENSOR_DEVICE_ATTR(i2c_lm75_4a_timeout,  S_IRUGO, get_i2c_timeout, NULL, 3);
...
static struct attribute *aster_hwmon_mcu_attributes[] = {
    ...
    &sensor_dev_attr_ADC9_under_alert.dev_attr.attr,
    ...
    &sensor_dev_attr_ADC9_over_alert.dev_attr.attr,
    ...
    &sensor_dev_attr_i2c_lm75_4a_timeout.dev_attr.attr,
    ...
    &sensor_dev_attr_ADC9_vol.dev_attr.attr,
...
static const struct attribute_group aster_hwmon_mcu_group = { .attrs = aster_hwmon_mcu_attributes};

static int aster_hwmon_mcu_probe(struct i2c_client *client,
            const struct i2c_device_id *dev_id)
{  
    int status;

    status = sysfs_create_group(&client->dev.kobj, &aster_hwmon_mcu_group);
...
static int aster_hwmon_mcu_remove(struct i2c_client *client)
{
    sysfs_remove_group(&client->dev.kobj, &aster_hwmon_mcu_group);
    return 0;
}

static const struct i2c_device_id aster_hwmon_mcu_id[] = {
    { "aster_hwmon_mcu", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, aster_hwmon_mcu_id);

static struct i2c_driver aster_hwmon_mcu_driver = {
    .class      = I2C_CLASS_HWMON,
    .driver = {
        .name = "aster_hwmon_mcu",
    },
    .probe      = aster_hwmon_mcu_probe,
    .remove     = aster_hwmon_mcu_remove,
    .id_table   = aster_hwmon_mcu_id,
};

static int __init aster_hwmon_mcu_init(void)
{
    mutex_init(&aster_hwmon_mcu_lock);

    return i2c_add_driver(&aster_hwmon_mcu_driver);
}

static void __exit aster_hwmon_mcu_exit(void)
{
    i2c_del_driver(&aster_hwmon_mcu_driver);
}

MODULE_AUTHOR("Wang Lin <wanglin@asterfusion.com>");
MODULE_DESCRIPTION("aster_hwmon_mcu driver");
MODULE_LICENSE("GPL");

module_init(aster_hwmon_mcu_init);
module_exit(aster_hwmon_mcu_exit);
