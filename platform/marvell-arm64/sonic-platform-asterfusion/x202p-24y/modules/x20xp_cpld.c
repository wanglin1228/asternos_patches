...
#define QSFP_STATUS                 0x06
#define BOARD_STAT                  0x07
#define RST1_CTL                    0x08
#define RST2_CTL                    0x09
#define SYS_LED                     0x0A
#define CPLD_CTL1                   0x0B
#define FAN1_INLET_RPM              0x0C
#define FAN1_OUTLET_RPM             0x0D
#define FAN2_INLET_RPM              0x0E
#define FAN2_OUTLET_RPM             0x0F
#define FAN3_INLET_RPM              0x10
#define FAN3_OUTLET_RPM             0x11
...
#define SFP1_STATUS                 0x1F
#define SFP2_STATUS                 0x20
#define SFP3_STATUS                 0x21
#define SFP1_RX_LOSS                0x22
#define SFP2_RX_LOSS                0x23
#define SFP3_RX_LOSS                0x24
#define SFP1_TX_DISABLE             0x25
#define SFP2_TX_DISABLE             0x26
#define SFP3_TX_DISABLE             0x27
...
// #define IO_MUX_RST_BIT      4 //Unused
// #define LED_STR_RST_BIT     3 //Unused
#define PCA9548_RST_BIT     2
#define GE_PHY_RST_BIT      1
#define SWITCH_RST_BIT      0

// #define QSFP28_6_RST_BIT    5 //Unused
// #define QSFP28_5_RST_BIT    4 //Unused
// #define QSFP28_4_RST_BIT    3 //Unused
// #define QSFP28_3_RST_BIT    2 //Unused
#define QSFP28_2_RST_BIT    1
#define QSFP28_1_RST_BIT    0


#define SFP28_1_TX_DISABLE_BIT 0
#define SFP28_2_TX_DISABLE_BIT 1
#define SFP28_3_TX_DISABLE_BIT 2
#define SFP28_4_TX_DISABLE_BIT 3
#define SFP28_5_TX_DISABLE_BIT 4
#define SFP28_6_TX_DISABLE_BIT 5
#define SFP28_7_TX_DISABLE_BIT 6
#define SFP28_8_TX_DISABLE_BIT 7
 ...
 */
static const unsigned short normal_i2c[] = { CPLD_ADDRESS, PCA9548_0X70, PCA9548_0X71, PCA9548_0X72, PCA9548_0X76, I2C_CLIENT_END };
...
int asterfusion_x20xp_cpld_reset(void)
{
    ...
    for(sfpBase = PCA9548_0X70; sfpBase < PCA9548_0X72 + 1; sfpBase++)
    { 
        asterfusion_x20xp_cpld_write(sfpBase, SFP_SCL_BASE, 0);
    }
    asterfusion_x20xp_cpld_write(PCA9548_0X76, SFP_SCL_BASE, 0);
...
static ssize_t qsfp_status_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (status & mask)
        {
            sprintf(buf, "%sSFP %02d is normal\n", buf, port_num);
        }
        else
        {
            sprintf(buf, "%sSFP %02d is abnormal\n", buf, port_num);
        }
        port_num++;
        mask = mask << 1;
    }
...
static ssize_t sfp_rx_loss_get(struct device *dev, struct device_attribute *da, char *buf)
{
    u32 status = -EPERM;
    u8 port_num = 1;
    u8 i;
...
static ssize_t sfp_tx_disable_get(struct device *dev, struct device_attribute *da, char *buf)
{
    u32 status = -EPERM;
    u8 port_num = 1;
    u8 i;
...
static ssize_t sfp1_tx_disable_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    u32 status = -EPERM;
    u16 val = simple_strtol(buf, NULL, 10);
    struct i2c_client *client = to_i2c_client(dev);
    ...
    status = asterfusion_x20xp_cpld_read_on_lock(client->addr, SFP1_TX_DISABLE);
    ...
    status = asterfusion_x20xp_cpld_write_on_lock(client->addr, SFP1_TX_DISABLE, status);
    return count;
}
static ssize_t sfp2_tx_disable_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    u32 status = -EPERM;
    u16 val = simple_strtol(buf, NULL, 10);
    struct i2c_client *client = to_i2c_client(dev);
    ...
    status = asterfusion_x20xp_cpld_read_on_lock(client->addr, SFP2_TX_DISABLE);
    ...
    status = asterfusion_x20xp_cpld_write_on_lock(client->addr, SFP2_TX_DISABLE, status);
    return count;
}
static ssize_t sfp3_tx_disable_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    u32 status = -EPERM;
    u16 val = simple_strtol(buf, NULL, 10);
    struct i2c_client *client = to_i2c_client(dev);
    ...
    status = asterfusion_x20xp_cpld_read_on_lock(client->addr, SFP3_TX_DISABLE);
    ...
    status = asterfusion_x20xp_cpld_write_on_lock(client->addr, SFP3_TX_DISABLE, status);
    return count;
}
/* end of function */
...
static SENSOR_DEVICE_ATTR(qsfp_status       , S_IRUGO            ,  qsfp_status_get   ,  NULL             , QSFP_STATUS );//0X06
...
// static SENSOR_DEVICE_ATTR(qsfp28_6_rst      , S_IRUGO | S_IWUSR , rst2_ctl_get       , rst2_ctl_set       , QSFP28_6_RST_BIT);
...
static SENSOR_DEVICE_ATTR(qsfp28_2_rst      , S_IRUGO | S_IWUSR , rst2_ctl_get       , rst2_ctl_set       , QSFP28_2_RST_BIT);//0x09
...
static SENSOR_DEVICE_ATTR(fan1_outlet_rpm   , S_IRUGO           , fan_rpm_get       , NULL              , FAN1_INLET_RPM);//0x0C
static SENSOR_DEVICE_ATTR(fan1_inlet_rpm    , S_IRUGO           , fan_rpm_get       , NULL              , FAN1_OUTLET_RPM);//0x0D
static SENSOR_DEVICE_ATTR(fan2_outlet_rpm   , S_IRUGO           , fan_rpm_get       , NULL              , FAN2_INLET_RPM);//0xE
static SENSOR_DEVICE_ATTR(fan2_inlet_rpm    , S_IRUGO           , fan_rpm_get       , NULL              , FAN2_OUTLET_RPM);//0x0F
static SENSOR_DEVICE_ATTR(fan3_outlet_rpm   , S_IRUGO           , fan_rpm_get       , NULL              , FAN3_INLET_RPM);//0x10
static SENSOR_DEVICE_ATTR(fan3_inlet_rpm    , S_IRUGO           , fan_rpm_get       , NULL              , FAN3_OUTLET_RPM);//0x11
...
static SENSOR_DEVICE_ATTR(sfp1_status       , S_IRUGO           ,  sfp_status_get    ,  NULL               , SFP1_STATUS );//0X1F
static SENSOR_DEVICE_ATTR(sfp2_status       , S_IRUGO           ,  sfp_status_get    ,  NULL               , SFP2_STATUS );//0X20
static SENSOR_DEVICE_ATTR(sfp3_status       , S_IRUGO           ,  sfp_status_get    ,  NULL               , SFP3_STATUS );//0X21
static SENSOR_DEVICE_ATTR(sfp1_rx_loss      , S_IRUGO           ,  sfp_rx_loss_get   ,  NULL               , SFP1_RX_LOSS);//0X22
static SENSOR_DEVICE_ATTR(sfp2_rx_loss      , S_IRUGO           ,  sfp_rx_loss_get   ,  NULL               , SFP2_RX_LOSS);//0X23
static SENSOR_DEVICE_ATTR(sfp3_rx_loss      , S_IRUGO           ,  sfp_rx_loss_get   ,  NULL               , SFP3_RX_LOSS);//0X24
static SENSOR_DEVICE_ATTR(sfp1_tx_disable   , S_IRUGO           ,  sfp_tx_disable_get,  NULL               , SFP1_TX_DISABLE);//0X25
static SENSOR_DEVICE_ATTR(sfp2_tx_disable   , S_IRUGO           ,  sfp_tx_disable_get,  NULL               , SFP2_TX_DISABLE);//0X26
static SENSOR_DEVICE_ATTR(sfp3_tx_disable   , S_IRUGO           ,  sfp_tx_disable_get,  NULL               , SFP3_TX_DISABLE);//0X27
static SENSOR_DEVICE_ATTR(sfp1_1_tx_disable , S_IWUSR           ,  NULL              ,  sfp1_tx_disable_set, SFP28_1_TX_DISABLE_BIT);//0X25
static SENSOR_DEVICE_ATTR(sfp1_2_tx_disable , S_IWUSR           ,  NULL              ,  sfp1_tx_disable_set, SFP28_2_TX_DISABLE_BIT);//0X25
static SENSOR_DEVICE_ATTR(sfp1_3_tx_disable , S_IWUSR           ,  NULL              ,  sfp1_tx_disable_set, SFP28_3_TX_DISABLE_BIT);//0X25
static SENSOR_DEVICE_ATTR(sfp1_4_tx_disable , S_IWUSR           ,  NULL              ,  sfp1_tx_disable_set, SFP28_4_TX_DISABLE_BIT);//0X25
static SENSOR_DEVICE_ATTR(sfp1_5_tx_disable , S_IWUSR           ,  NULL              ,  sfp1_tx_disable_set, SFP28_5_TX_DISABLE_BIT);//0X25
static SENSOR_DEVICE_ATTR(sfp1_6_tx_disable , S_IWUSR           ,  NULL              ,  sfp1_tx_disable_set, SFP28_6_TX_DISABLE_BIT);//0X25
static SENSOR_DEVICE_ATTR(sfp1_7_tx_disable , S_IWUSR           ,  NULL              ,  sfp1_tx_disable_set, SFP28_7_TX_DISABLE_BIT);//0X25
static SENSOR_DEVICE_ATTR(sfp1_8_tx_disable , S_IWUSR           ,  NULL              ,  sfp1_tx_disable_set, SFP28_8_TX_DISABLE_BIT);//0X25
static SENSOR_DEVICE_ATTR(sfp2_9_tx_disable , S_IWUSR           ,  NULL              ,  sfp2_tx_disable_set, SFP28_1_TX_DISABLE_BIT);//0X26
static SENSOR_DEVICE_ATTR(sfp2_10_tx_disable, S_IWUSR           ,  NULL              ,  sfp2_tx_disable_set, SFP28_2_TX_DISABLE_BIT);//0X26
static SENSOR_DEVICE_ATTR(sfp2_11_tx_disable, S_IWUSR           ,  NULL              ,  sfp2_tx_disable_set, SFP28_3_TX_DISABLE_BIT);//0X26
static SENSOR_DEVICE_ATTR(sfp2_12_tx_disable, S_IWUSR           ,  NULL              ,  sfp2_tx_disable_set, SFP28_4_TX_DISABLE_BIT);//0X26
static SENSOR_DEVICE_ATTR(sfp2_13_tx_disable, S_IWUSR           ,  NULL              ,  sfp2_tx_disable_set, SFP28_5_TX_DISABLE_BIT);//0X26
static SENSOR_DEVICE_ATTR(sfp2_14_tx_disable, S_IWUSR           ,  NULL              ,  sfp2_tx_disable_set, SFP28_6_TX_DISABLE_BIT);//0X26
static SENSOR_DEVICE_ATTR(sfp2_15_tx_disable, S_IWUSR           ,  NULL              ,  sfp2_tx_disable_set, SFP28_7_TX_DISABLE_BIT);//0X26
static SENSOR_DEVICE_ATTR(sfp2_16_tx_disable, S_IWUSR           ,  NULL              ,  sfp2_tx_disable_set, SFP28_8_TX_DISABLE_BIT);//0X26
static SENSOR_DEVICE_ATTR(sfp3_17_tx_disable, S_IWUSR           ,  NULL              ,  sfp3_tx_disable_set, SFP28_1_TX_DISABLE_BIT);//0X27
static SENSOR_DEVICE_ATTR(sfp3_18_tx_disable, S_IWUSR           ,  NULL              ,  sfp3_tx_disable_set, SFP28_2_TX_DISABLE_BIT);//0X27
static SENSOR_DEVICE_ATTR(sfp3_19_tx_disable, S_IWUSR           ,  NULL              ,  sfp3_tx_disable_set, SFP28_3_TX_DISABLE_BIT);//0X27
static SENSOR_DEVICE_ATTR(sfp3_20_tx_disable, S_IWUSR           ,  NULL              ,  sfp3_tx_disable_set, SFP28_4_TX_DISABLE_BIT);//0X27
static SENSOR_DEVICE_ATTR(sfp3_21_tx_disable, S_IWUSR           ,  NULL              ,  sfp3_tx_disable_set, SFP28_5_TX_DISABLE_BIT);//0X27
static SENSOR_DEVICE_ATTR(sfp3_22_tx_disable, S_IWUSR           ,  NULL              ,  sfp3_tx_disable_set, SFP28_6_TX_DISABLE_BIT);//0X27
static SENSOR_DEVICE_ATTR(sfp3_23_tx_disable, S_IWUSR           ,  NULL              ,  sfp3_tx_disable_set, SFP28_7_TX_DISABLE_BIT);//0X27
static SENSOR_DEVICE_ATTR(sfp3_24_tx_disable, S_IWUSR           ,  NULL              ,  sfp3_tx_disable_set, SFP28_8_TX_DISABLE_BIT);//0X27
...
static struct attribute *X20XP_SFP_attributes[] = 
{
    &sensor_dev_attr_qsfp_status.dev_attr.attr,
    &sensor_dev_attr_sfp1_status.dev_attr.attr,
    &sensor_dev_attr_sfp2_status.dev_attr.attr,
    &sensor_dev_attr_sfp3_status.dev_attr.attr,
    ...
    &sensor_dev_attr_sfp1_1_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp1_2_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp1_3_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp1_4_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp1_5_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp1_6_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp1_7_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp1_8_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp2_9_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp2_10_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp2_11_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp2_12_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp2_13_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp2_14_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp2_15_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp2_16_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp3_17_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp3_18_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp3_19_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp3_20_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp3_21_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp3_22_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp3_23_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp3_24_tx_disable.dev_attr.attr,
...
static const struct attribute_group X20XP_SFP_group =
{
    .name  = "X202P_24Y_SFP",
    .attrs = X20XP_SFP_attributes,
};
...
{  
    ...
    switch(client->addr)
    {
        case CPLD_ADDRESS:
            ...
            status = sysfs_create_group(&client->dev.kobj, &X20XP_SFP_group);
            break;
        case PCA9548_0X70:
        case PCA9548_0X71:
        case PCA9548_0X72:
...
static int asterfusion_x20xp_cpld_remove(struct i2c_client *client)
{
    switch(client->addr)
    {
        case CPLD_ADDRESS:
            ...
            sysfs_remove_group(&client->dev.kobj, &X20XP_SFP_group);
            break;
        case PCA9548_0X70:
        case PCA9548_0X71:
        case PCA9548_0X72:
