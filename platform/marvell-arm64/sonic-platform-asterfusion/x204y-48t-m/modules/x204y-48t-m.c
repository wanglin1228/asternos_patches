/* An hwmon driver for Asterfusion X204Y-48T-M CPLD I2C Module */
#pragma GCC diagnostic ignored "-Wformat-zero-length"
#include "x204y-48t-m.h"
...
static ssize_t cpld_byte_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(x204y_48t_i2c_client, attr->index);
    }
...
static ssize_t sfp_tx_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(x204y_48t_i2c_client, SFP_TX_STAT);
    ...
    {
        case SFP52_BIT:
        sprintf(buf, "%sSFP52 tx_disable =  0x%x\n", buf, value);
        break;
        case SFP51_BIT:
        sprintf(buf, "%sSFP51 tx_disable =  0x%x\n", buf, value);
        break;
        case SFP50_BIT:
        sprintf(buf, "%sSFP50 tx_disable =  0x%x\n", buf, value);
        break;
        case SFP49_BIT:
        sprintf(buf, "%sSFP49 tx_disable =  0x%x\n", buf, value);
        break;
    }
    return sprintf(buf, "%s\n", buf);
}
static ssize_t sfp_tx_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if(val)
        SET_BIT(status, (attr->index % 4));
    else
        CLEAR_BIT(status, (attr->index % 4));

    status = i2c_smbus_write_byte_data(x204y_48t_i2c_client, SFP_TX_STAT, status);
    return count;
}
...
static ssize_t rst_ctl_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(x204y_48t_i2c_client, RST_CTL);
    GET_BIT(status, attr->index, value);
    ...
    switch(attr->index)
    {
        ...
        break;
        case PHY6_CTL_BIT:
        sprintf(buf, "%sRESET_PHY6 status =  0x%x\n", buf, value);
        break;
        case PHY5_CTL_BIT:
        sprintf(buf, "%sRESET_PHY5 status =  0x%x\n", buf, value);
        break;
        case PHY4_CTL_BIT:
        sprintf(buf, "%sRESET_PHY4 status =  0x%x\n", buf, value);
        break;
        case PHY3_CTL_BIT:
        sprintf(buf, "%sRESET_PHY3 status =  0x%x\n", buf, value);
        break;
        case PHY2_CTL_BIT:
...
static ssize_t rst_ctl_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    status = i2c_smbus_write_byte_data(x204y_48t_i2c_client, RST_CTL, status);
    return count;
}
...
static ssize_t cpld_ctrl1_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(x204y_48t_i2c_client, CPLD_CTL1);
    GET_BIT(status, attr->index, value);
...
static ssize_t cpld_ctrl1_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    status = i2c_smbus_write_byte_data(x204y_48t_i2c_client, CPLD_CTL1, status);
    return count;
}
static ssize_t fan_rpm_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (FAN1_SPEED_RPM == attr->index || FAN2_SPEED_RPM == attr->index || FAN3_SPEED_RPM == attr->index)
    {
        status = i2c_smbus_read_byte_data(x204y_48t_i2c_client, attr->index);
...
static ssize_t fan_stat_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(x204y_48t_i2c_client, FAN_STAT);
    GET_BIT(status, attr->index, value);
...
static ssize_t fan_mode_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (FAN_MODE == attr->index)
    {
        status = i2c_smbus_write_byte_data(x204y_48t_i2c_client, attr->index, val & 0x1);
    }
...
static ssize_t fan_speed_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (FAN_SPEED == attr->index)
    {
        status = i2c_smbus_write_byte_data(x204y_48t_i2c_client, attr->index, val % 8);
    }
...
static ssize_t sys_led_ctrl_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(x204y_48t_i2c_client, SYS_LED); //to get register 0x30 0x13
        status &= 0x3;
        debug_print((KERN_DEBUG "DEBUG : sys_led_ctrl_get led status = %x\n",status));

        if(status == LED_AMBER_BLINK)
        {
            sprintf(buf, "%samber and blink\n", buf);
        }
        else if(status == LED_AMBER)
        {
            sprintf(buf, "%samber\n", buf);
        }
        else if(status == LED_GREEN_BLINK)
        {
            sprintf(buf, "%sgreen and blink\n", buf);
        }
        else if(status == LED_GREEN_AMBER)
        {
            sprintf(buf, "%sgreen and amber\n", buf);
        }
    }
...
static ssize_t sys_led_ctrl_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (SYS_LED == attr->index)
    {
        status = i2c_smbus_write_byte_data(x204y_48t_i2c_client, attr->index, val & 0x3);
    }
    return count;
}
...
static ssize_t hw_reset_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    sprintf(buf, "HW_RESET is set to: ");
    if (HW_RESET == attr->index)
    {
        status = i2c_smbus_read_byte_data(x204y_48t_i2c_client, HW_RESET);
        status &= 0x1;
...
static int Asterfusion_i2c_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
    ...
    status = sysfs_create_group(&client->dev.kobj, &X204Y_48T_SYS_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X204Y_48T_Reset_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X204Y_48T_Sensor_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X204Y_48T_Led_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X204Y_48T_SFP_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &X204Y_48T_FAN_group);
    if (status)
    {
        goto exit_free;
    }
    ...
    return 0;
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &X204Y_48T_SYS_group);
    sysfs_remove_group(&client->dev.kobj, &X204Y_48T_Reset_group);
    sysfs_remove_group(&client->dev.kobj, &X204Y_48T_Sensor_group);
    sysfs_remove_group(&client->dev.kobj, &X204Y_48T_Led_group);
    sysfs_remove_group(&client->dev.kobj, &X204Y_48T_SFP_group);
    sysfs_remove_group(&client->dev.kobj, &X204Y_48T_FAN_group);
...
static struct i2c_board_info X204Y_48T_i2c_info[] __initdata =
{
    {
        I2C_BOARD_INFO("x204y_48t_m_i2c", 0x30),
        .platform_data = NULL,
    },
};
...
static int __init Asterfusion_i2c_init(void)
{
    ...
    x204y_48t_i2c_client = i2c_new_client_device(i2c_adap, &X204Y_48T_i2c_info[0]);
    if (x204y_48t_i2c_client == NULL)
    {
        printk("ERROR: x204y_48t_i2c_client FAILED!\n");
        return -1;
    }
...
{
    i2c_unregister_device(x204y_48t_i2c_client);
    i2c_del_driver(&Asterfusion_i2c_driver);
}

MODULE_AUTHOR("Asterfusion Inc.");
MODULE_DESCRIPTION("Asterfusion X204Y-48T-M i2c Driver");
MODULE_LICENSE("GPL");
