...
struct Asterfusion_uart_data *Asterfusion_Uart; 
struct i2c_client *CX_308P_i2c_client; //0x40 for all device
...
int uart_write_cmd(u8 cmd, u8 sub_cmd_1, u8 sub_cmd_2, u8 n_para)
{
	...
	if(cmd > 0x10 || !Asterfusion_Uart->valid)
		return -EPERM;
	
    mutex_lock(&Asterfusion_Uart->update_lock);	
    ...
    mutex_unlock(&Asterfusion_Uart->update_lock);
...
int uart_read_cmd(u8 cmd, u8 sub_cmd_1, u8 sub_cmd_2, u8 n_para, u8 *r_data)
{
	...
	if(r_data == NULL || !Asterfusion_Uart->valid)
		return -EINVAL;
	...
	do{
		while(!(inb(UART_TTYS1_OFFSET + UART_LSR) & UART_LSR_TEMT))
			usleep_range(50,100);
		outb(uart_order[w_data_num] & 0xff, UART_TTYS1_OFFSET);
		w_data_num++;
...
static int Asterfusion_uart_probe(void)
{
	Asterfusion_Uart->iobase = UART_TTYS1_OFFSET;
	Asterfusion_Uart->sirq = UART_TTYS1_SIRQ;
	disable_irq(Asterfusion_Uart->sirq);
	...
	Asterfusion_Uart->valid = 1;
	return 0;
}

static int Asterfusion_cx308p_e02_device_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
    ...
    mutex_init(&Asterfusion_Uart->update_lock);

	Asterfusion_uart_probe();
...
static int Asterfusion_cx308p_e02_device_remove(struct i2c_client *client)
{
...
static const struct i2c_device_id Asterfusion_cx308p_e02_i2c_id[] =
{
    { "CX308P_e02_i2c", 2 },
    {},
};
MODULE_DEVICE_TABLE(i2c, Asterfusion_cx308p_e02_i2c_id);

static struct i2c_driver Asterfusion_cx308p_e02_i2c_driver =
{
    ...
    .probe        = Asterfusion_cx308p_e02_device_probe,
    .remove       = Asterfusion_cx308p_e02_device_remove,
    .id_table     = Asterfusion_cx308p_e02_i2c_id,
    .address_list = normal_i2c,
};

static struct i2c_board_info CX308P_e02_i2c_info[] __initdata =
{
    {
        I2C_BOARD_INFO("CX308P_e02_i2c", 0x40),
        .platform_data = NULL,
    },
};


static int __init Asterfusion_cx308p_e02_module_init(void)
{
    int ret;
    ...
    Asterfusion_Uart = kzalloc(sizeof(struct Asterfusion_uart_data), GFP_KERNEL);
    if (!Asterfusion_Uart)
    {
        printk(KERN_ALERT "Asterfusion_Uart kzalloc FAILED\n");
        return -1;
    }
    CX_308P_i2c_client = i2c_new_client_device(i2c_adap, &CX308P_e02_i2c_info[0]);
    ...
    ret = i2c_add_driver(&Asterfusion_cx308p_e02_i2c_driver);
    printk(KERN_ALERT "CX308P-48Y-N i2c Driver Version: %s\n", DRIVER_VERSION);
    printk(KERN_ALERT "CX308P-48Y-N i2c Driver INSTALL SUCCESS\n");
    return ret;
}

static void __exit Asterfusion_cx308p_e02_module_exit(void)
{
    enable_irq(UART_TTYS1_SIRQ);
	i2c_unregister_device(CX_308P_i2c_client);
    i2c_del_driver(&Asterfusion_cx308p_e02_i2c_driver);
	kfree(Asterfusion_Uart);
...
/* To driver developer: 
 *     Main variable names of this driver has been added a unique prefix(e.g. "e02").
 ...
 */
module_init(Asterfusion_cx308p_e02_module_init);
module_exit(Asterfusion_cx308p_e02_module_exit);
